...
const dedupeReasons = (reasons: DecisionReason[]) => {
...
const fallbackReason = (
  code: string,
  source: DecisionReason["source"] = "derived",
): DecisionReason => ({
  code,
...
export const buildGoalFitCard = (input: {
  ...
  const whyFit = dedupeReasons(
    (match?.reasons ?? []).filter((reason) => POSITIVE_REASON_CODES.has(reason.code)),
  );
  ...
  return {
    ...
    confidence: buildConfidenceBreakdown({
      evaluation,
    ...
    whyNotStronger:
      whyNotStronger.length > 0 ? whyNotStronger : [fallbackReason("goal_fit_no_major_strength_caps")],
    holdbacks:
      holdbacks.length > 0
        ? holdbacks
        : evaluation.coverage.status === "coverage_ready"
