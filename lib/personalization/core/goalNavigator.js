...
exports.goalNavigatorInternals = exports.buildGoalNavigatorResponse = void 0;
const TIER_PRIORITY = {
    strong_match: 4,
    related: 3,
    weak_match: 2,
...
const buildReason = (code, params) => ({
    code,
    ruleId: "personalization.goal_navigator.v1",
    source: "derived",
    ...(params ? { params } : {}),
});
const isRankEligible = (candidate) => candidate.evaluation.coverage.status === "coverage_ready" &&
    (candidate.evaluation.eligibility?.rankEligible ?? true) &&
    candidate.tier !== "no_match";
const compareByDefaultRanking = (left, right) => {
    ...
    if (scoreDelta !== 0)
        return scoreDelta;
    const evidenceDelta = EVIDENCE_PRIORITY[right.goalFitCard.confidence.evidence] -
        EVIDENCE_PRIORITY[left.goalFitCard.confidence.evidence];
    if (evidenceDelta !== 0)
        return evidenceDelta;
    const overlapDelta = OVERLAP_PRIORITY[right.goalFitCard.confidence.overlapRisk] -
        OVERLAP_PRIORITY[left.goalFitCard.confidence.overlapRisk];
    if (overlapDelta !== 0)
        return overlapDelta;
    const routineDelta = ROUTINE_PRIORITY[right.goalFitCard.confidence.routineFit] -
...
const sortCandidates = (candidates, preferenceVector) => [...candidates].sort((left, right) => {
    switch (preferenceVector?.decisionMode) {
        ...
        case "better_disclosure": {
            const labelDelta = LABEL_PRIORITY[right.goalFitCard.confidence.labelCompleteness] -
...
const buildGoalNavigatorResponse = (input) => {
    const sortedCandidates = sortCandidates(input.candidates.filter(isRankEligible), input.preferenceVector);
    const strongOnlyCandidates = input.preferenceVector?.decisionMode === "strong_only"
        ? sortedCandidates.filter((candidate) => candidate.tier === "strong_match")
        : sortedCandidates;
    const rankableCandidates = (strongOnlyCandidates.length > 0 ? strongOnlyCandidates : sortedCandidates).slice(0, Math.max(1, input.limit ?? 6));
    return {
...
exports.buildGoalNavigatorResponse = buildGoalNavigatorResponse;
exports.goalNavigatorInternals = {
    buildReason,
