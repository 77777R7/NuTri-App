...
Modes:
  start, run        Start the Expo Go dev server
  --ios, ios        Start Expo Go and open iOS
  --android, android
                   Start Expo Go and open Android
  --web, web        Start Expo for web
  --dev-client, dev-client
                   Start Expo in development-client mode
  --tunnel, tunnel Start Expo Go using tunnel transport
...
case "$MODE" in
  start|run)
    exec "${EXPO_CMD[@]}" start --go
    ;;
  --ios|ios)
    exec "${EXPO_CMD[@]}" start --go --ios
    ;;
  --android|android)
    exec "${EXPO_CMD[@]}" start --go --android
    ;;
  --web|web)
