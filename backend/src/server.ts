...
const buildBootstrapProductIdentityFromDigest = (params: {
  digest: FactsDigest;
  identityType: FactsDigest["identity"]["type"];
  identityValue: string;
}): NonNullable<AnalysisBundle["meta"]["productIdentity"]> | null => {
  const name = safeTrim(params.digest.product?.name ?? null);
  const brand = safeTrim(params.digest.product?.brandDisplay ?? null);
  const sourceId = safeTrim(params.identityValue);
  if (!name && !brand && !sourceId) return null;
  const verifiedRegulatory = params.digest.sourceType === "dsld" || params.digest.sourceType === "lnhpd";
  return {
    name,
    brand,
    sourceAttribution: verifiedRegulatory ? "verified_regulatory" : "web_hint_unverified",
    identityStable: verifiedRegulatory ? Boolean(name) : false,
    sourceId: sourceId ? `${params.identityType}:${sourceId}` : null,
  };
};

const buildScanBootstrapSources = (params: {
  overlayClaims: DecisionSupportOverlayClaims | null;
}): Array<{ title: string; link: string; domain?: string; isHighQuality?: boolean }> => {
  const sources: Array<{ title: string; link: string; domain?: string; isHighQuality?: boolean }> = [];
  const overlayLink = safeTrim(params.overlayClaims?.link ?? null);
  if (overlayLink) {
    sources.push({
      title: "iHerb product page",
      link: overlayLink,
      domain: extractDomain(overlayLink),
      isHighQuality: true,
    });
  }
  return sources;
...
const pickBootstrapText = (...values: Array<string | null | undefined>): string | null => {
  for (const value of values) {
    const trimmed = safeTrim(value ?? null);
    if (trimmed) return trimmed;
  }
  return null;
};

app.get("/api/scan-bootstrap/v1", verifySupabaseToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const digest = authority.patched.digest;
    const identityType = digest.identity.type;
    const identityValue = digest.identity.value;
    const deterministicSignals = DETERMINISTIC_SIGNALS_PRIMARY
      ...
      : null;
    const productIdentity = buildBootstrapProductIdentityFromDigest({
      digest,
      identityType,
      identityValue,
    });
    const skeletonBase = buildAnalysisBundleSkeleton({
      ...
      bundleId: `bootstrap:${authority.barcodeGtin14}:${Date.now()}`,
      revision: 0,
      phase: "skeleton",
      locale: "en",
      factsDigestHash: authority.quickDigest.factsDigestHash,
      factsSourceVersion: authority.quickDigest.factsSourceVersion,
      identityType,
      identityValue,
      dataStatus: {
        overview: "limited",
      ...
      overlayClaims: authority.overlayClaims,
    });
    const skeleton = {
      ...skeletonBase,
      meta: {
        ...skeletonBase.meta,
        productIdentity: productIdentity ?? skeletonBase.meta.productIdentity,
      },
    } satisfies AnalysisBundle;
    const deterministicFastBundle = applyFastFailureStatus(
      applyDsldInferenceGuard(
        sanitizeAnalysisBundleCoverFields({
          bundle: mergeFastAnalysisBundle({
            skeleton,
            digest,
            deterministicSignals,
            fastOutput: null,
    ...
    const parsedBundle = safeParseAnalysisBundle(deterministicFastBundle);
    if (!parsedBundle.success) {
      console.warn("[scan-bootstrap] bundle validation failed", parsedBundle.error?.message);
      return res
        .status(503)
        .json({ error: "bootstrap_unavailable", detail: "bundle_validation_failed" } satisfies ErrorResponse);
    }

    const productInfo = {
      brand: pickBootstrapText(digest.product.brandDisplay ?? null, authority.overlayClaims?.brandName ?? null),
      name: pickBootstrapText(digest.product.name ?? null, authority.overlayClaims?.title ?? null),
      category: null,
      image: pickScanResultProductImage(authority.barcodeGtin14),
    };
    const analysisMeta = {
      status: "label_enriched" as const,
      version: 4,
      labelExtraction: {
        source:
          digest.sourceType === "dsld"
            ? "dsld" as const
            : digest.sourceType === "lnhpd"
              ? "lnhpd" as const
              : "manual" as const,
        fetchedAt: null,
        datasetVersion: authority.quickDigest.factsSourceVersion,
      },
    };
    ...
    return res.json({
      ...
      barcode: normalizedBarcode.code,
      productInfo,
      sources: buildScanBootstrapSources({ overlayClaims: authority.overlayClaims }),
      analysisMeta,
      analysisBundle: parsedBundle.data,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    console.error("[scan-bootstrap] failed", message);
    return res
      .status(503)
      .json({ error: "bootstrap_unavailable", detail: message } satisfies ErrorResponse);
  }
});
