...
const ID_COLUMN = (getArg("id-column") ?? "source_id").trim();
const OUTPUT = getArg("output") ?? "output/formraw/formraw_nonempty_diff_dsld.json";
const COMPARE_SNAPSHOT = getArg("compare-snapshot");
const SAMPLE_LIMIT = Number.isFinite(Number(getArg("sample-limit") ?? "200"))
  ? Number(getArg("sample-limit") ?? "200")
  : 200;
  ...
  for (const chunk of chunkArray(sourceIds, CHUNK_SIZE)) {
    const { data, error } = await withRetry(() =>
      supabase
        ...
        .select("id,source_id,canonical_source_id,ingredient_id,name_raw,form_raw,name_key,is_active")
        .eq("source", "dsld")
        .eq("is_active", true)
        .not("ingredient_id", "is", null)
        .not("form_raw", "is", null)
        .neq("form_raw", "")
        .in(idColumn as any, chunk as any),
    );
    if (error) {
      const meta = extractErrorMeta(error);
      throw new Error(`[formraw-nonempty-diff] query failed: ${meta.message ?? "unknown error"}`);
    }
...
const fetchRowsByIds = async (rowIds: string[]): Promise<IngredientRow[]> => {
  ...
  for (const chunk of chunkArray(rowIds, CHUNK_SIZE)) {
    ...
    if (error) {
      const meta = extractErrorMeta(error);
      throw new Error(`[formraw-nonempty-diff] id fetch failed: ${meta.message ?? "unknown error"}`);
    }
    rows.push(...((data ?? []) as IngredientRow[]));
  }
  return rows;
