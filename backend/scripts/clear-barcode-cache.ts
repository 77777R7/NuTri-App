...
const logCounts = (label: string, data: unknown) => {
...
const main = async () => {
  ...
  if (snapshotRow && typeof snapshotRow === "object") {
    ...
    const normalizedBarcode = payload?.product?.barcode?.normalized ? String(payload.product.barcode.normalized) : null;
    ...
    if (normalizedBarcode && normalizedBarcode.length === 14) identities.push({ type: "gtin14", value: normalizedBarcode });
