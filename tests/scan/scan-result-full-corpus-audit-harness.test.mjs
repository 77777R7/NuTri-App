...
test("scientific background fallback pending is not P0 for optional metadata or bounded claim language", () => {
  const product = {
    family: "cla",
    productName: "CLA 1000 mg",
    brand: "Brand",
    activeIngredientNames: ["CLA"],
  };
  const row = evaluateAiSummary({
    type: "scientific_background",
    product,
    payload: {
      mode: "research_mode",
      selectedLabel: "CLA",
      selectedDose: null,
      introLine: "CLA research is easiest to read through a label-specific fatty-acid lane with clear boundaries.",
      sections: [
        {
          heading: "Research Snapshot",
          summary:
            "CLA is easiest to read through body-composition and fatty-acid context rather than broad slimming claims.",
          bullets: [
            "Compare the exact CLA row and amount before treating broad body-composition wording as the whole story.",
            "This is a comparison boundary, not a treatment or prevention promise.",
            "Do not present this lane as treating disease or replacing medication.",
            "Treat citrate and oxide as different comparison lanes when the label discloses form.",
          ],
          evidenceRead:
            "Evidence is mixed and should stay narrower than generic weight-loss marketing claims.",
          shopperMeaning:
            "What this means when comparing: use the exact row and amount before relying on broader packaging language.",
        },
      ],
      closingNote: null,
    },
    source: "fallback",
    fallbackUsed: true,
    fallbackReason: "background_refresh_pending",
    backgroundRefreshPending: true,
    recommendedRetryAfterMs: 1000,
  });

  assert.notEqual(row.severity, "P0");
  assert.notEqual(row.severity, "P1");
  assert.equal(row.overclaimScore, 0);
  assert.deepEqual(row.blankFields, []);
  assert.equal(row.llm_timeout, false);
  assert.equal(row.backgroundRefreshPending, true);
});

test("ingredient overview background pending fallback is not counted as timeout or low-value failure", () => {
  const product = {
    family: "magnesium",
    productName: "Magnesium Bisglycinate",
    brand: "Brand",
    activeIngredientNames: ["Magnesium"],
  };
  const row = evaluateAiSummary({
    ...
    payload: {
      ...
      paragraph1:
        "Magnesium Bisglycinate is the main disclosed ingredient rather than one part of a broad formula.",
      paragraph2:
        "That makes the label easier to compare because the core active is named directly.",
      compareHint:
        "When comparing products, check the disclosed magnesium form and whether the label separates active magnesium from compound weight.",
    },
    source: "fallback",
    fallbackUsed: true,
    fallbackReason: "background_refresh_pending",
