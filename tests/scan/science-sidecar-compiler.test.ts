...
test("ingredient overview strips shopper-purpose drift and returns repaired api copy", async () => {
  ...
  assert.ok(!/people take this/i.test(result.ingredientOverview.paragraph1));
          ...
          sections: [
            {
              ...
              summary:
                "Astaxanthin is a naturally occurring carotenoid pigment.",
              bullets: [
            ...
            },
            {
              ...
              summary:
                "It has been studied for eye- and skin-related outcomes.",
            ...
            },
            {
              ...
              summary:
                "It has also been explored in exercise-related research.",
              bullets: [
...
test("scientific background uses phage-specific label-context fallback for phage blend lines", async () => {
  const phageContext = buildIngredientScienceContext({
    digest: buildDigest({
      ...
      dosageForm: "Capsule",
      actives: [{ name: "TetraPhage Blend", amount: 15, unit: "mg" }],
    }),
    overlayClaims: null,
  });
...
test("scientific background repairs template-style research prose into family-specific copy", async () => {
          ...
          sections: [
            {
              ...
              summary:
                "EPA is frequently studied in heart-related and triglyceride-related research.",
              bullets: [
                "Heart-related research",
                "Triglyceride-related research",
              ],
              evidenceRead: "Evidence varies.",
            ...
            },
            {
              ...
              summary:
                "EPA is also studied in inflammation-related and recovery-related research.",
              bullets: [
            ...
            },
            {
              ...
              summary:
                "EPA is often discussed in broader cardiovascular research.",
              bullets: [
                "Heart-related research",
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /clearest evidence lane|triglyceride/i,
  );
  assert.match(
    result.scientificBackground.sections[0]?.shopperMeaning ?? "",
    /EPA breakdown line|compare/i,
  );
  assert.equal(
        ...
        JSON.stringify({
          ...
          sections: [
            ...
            },
            {
              ...
              heading: "Inflammation and recovery context",
              summary:
                "EPA has also been explored in inflammation-related and recovery-related research.",
              bullets: [
                "Inflammation-related research",
                "Recovery-related research",
              ],
            ...
            },
            {
              ...
              evidenceRead: "Study designs vary.",
              shopperMeaning:
                "Compare the EPA line, not just the broad packaging language.",
            },
          ],
...
test("scientific background rescues loose json writer output into an api result", async () => {
  const digest = buildDigest({
    labelId: "fixture-epa-loose-json-repair",
    productName: "Omega-3 1040 mg Fish Oil 1250 mg",
    dosageForm: "Softgel",
    actives: [
      { name: "EPA (Eicosapentaenoic Acid)", amount: 690, unit: "mg" },
...
Here is the JSON:
{
  ...
  sections: [
    {
      heading_id: 'lipid_and_triglyceride_research',
      heading: 'Lipid and triglyceride research',
      summary: 'EPA is frequently studied in triglyceride-related and lipid-related research.',
      bullets: 'Triglyceride endpoints matter more than broad heart packaging. | This lane is clearer than umbrella cardiovascular language.',
      evidence_read: 'Evidence varies.',
      shopper_meaning: 'Compare the EPA breakdown line before broader heart language.'
    },
    {
      heading_id: 'inflammation_and_recovery_context',
      heading: 'Inflammation and recovery context',
      summary: 'EPA also appears in narrower inflammation-related and recovery-related discussions.',
      bullet1: 'This is usually a secondary lane.',
      bullet2: 'Do not rank products on this lane alone.',
      evidence_read: 'Results are mixed.',
      what_this_means: 'Use this as supporting context after the EPA disclosure line.'
    },
    {
      heading_id: 'broader_heart_claim_boundaries',
      heading: 'How this differs from broader heart claims',
      summary: 'Broad cardiovascular language is wider than the clearest EPA-specific lane.',
      bullets: 'Broad heart wording can overread the label. | EPA and DHA disclosure still matters most for comparison.',
      evidence: 'Study designs vary.',
      comparisonMeaning: 'Use packaging language as context, then go back to the exact EPA and DHA lines.'
    }
  ],
  closing_note: 'Read this as comparison context, not as a blanket promise.'
}`,
    },
  );
  ...
  assert.match(
    result.scientificBackground.sections[1]?.shopperMeaning ?? "",
    /supporting context|EPA disclosure|secondary comparison lens|EPA dose/i,
  );
});
...
test("scientific background can repair a partial writer response by filling missing planned sections from family fallback", async () => {
  ...
  const result = await compileScientificBackgroundAsync(context, "Vitamin C", {
    llmFn: async () =>
      JSON.stringify({
        introLine:
          "Vitamin C research often centers on antioxidant and tissue-support questions.",
        sections: [
          {
            headingId: "antioxidant_and_immune_research",
            heading: "Antioxidant and immune research",
            summary:
              "Vitamin C is often discussed in antioxidant and immune-related research.",
            bullets: [
            ...
            evidenceRead: "Evidence varies.",
            shopperMeaning:
              "Look at the exact ingredient and broader label context.",
          },
          {
            headingId: "collagen_and_tissue_support",
            heading: "Collagen and tissue support",
            summary:
              "Vitamin C also appears in collagen and tissue-support research.",
            bullets: ["Collagen-related research", "Tissue-support research"],
            evidenceRead: "Results are mixed.",
            shopperMeaning:
              "This helps explain why it shows up in more than one product category.",
          },
        ],
  ...
  assert.match(
    ...
    /iron co-administration|narrower but important vitamin c context/i,
  );
});
...
test("astaxanthin fallback keeps evidence texture instead of generic research bullets", async () => {
  ...
  assert.match(
    antioxidantSection?.summary ?? "",
    /oxidative-stress markers|antioxidant-response/i,
  );
  assert.match(
    eyeSkinSection?.summary ?? "",
    /visual fatigue|hydration|elasticity|UV-exposure/i,
  );
...
test("scientific background accepts magnesium live-style output when form and comparison meaning stay specific", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[1]?.summary ?? "",
    /form disclosure|glycinate|citrate|oxide/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /comparison set|form line|per-serving amount/i,
  );
});
...
test("scientific background accepts vitamin D live-style output when bone context stays primary and broader claims stay bounded", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /bone and calcium-regulation|clearest lane/i,
  );
  assert.match(
    result.scientificBackground.sections[1]?.shopperMeaning ?? "",
    /broad immune-style wording|rank/i,
  );
});
...
test("scientific background accepts calcium live-style output when bone context stays primary and form comparison stays careful", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[1]?.summary ?? "",
    /citrate|carbonate|form matters/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /comparison bucket|broader formula/i,
  );
});
...
test("scientific background accepts iron live-style output when supplementation context stays primary and form remains practical", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /supplementation and status-related lane|broad energy-style marketing/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /paired-nutrient context|comparison goal/i,
  );
});
...
test("scientific background live prompt includes reviewed evidence grounding when evidence-backed section rows exist", async () => {
  ...
  assert.match(
    capturedPrompt,
    /A systematic review, meta-analysis and meta-regression of the effect of protein supplementation/i,
  );
  assert.match(
    capturedPrompt,
    /When a label discloses source and grams clearly/i,
  );
});
...
test("scientific background accepts melatonin live-style output when timing context stays primary and dose framing remains practical", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /sleep timing and onset|circadian|timing/i,
  );
  assert.match(
    result.scientificBackground.sections[1]?.shopperMeaning ?? "",
    /dose|interchangeable|comparison set/i,
  );
});
...
test("scientific background accepts b12 live-style output when supplementation context stays primary and form disclosure remains comparison-oriented", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /supplementation and status-related|energy-style/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
...
test("scientific background accepts folate live-style output when developmental context stays specific and form labeling remains practical", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[1]?.summary ?? "",
    /developmental|pregnancy-related context/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /close substitutes|form|amount/i,
  );
});
...
test("scientific background accepts b6 live-style output when cofactor context stays primary and dose meaning remains practical", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /cofactor and metabolism|energy-style/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /comparison bucket|dose|formula role/i,
  );
});
...
test("scientific background accepts curcumin live-style output when extract detail stays comparison-oriented", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[1]?.summary ?? "",
    /extract|curcuminoid|standardized/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /extract detail|broadest promise|shopping decision/i,
  );
});
...
test("scientific background accepts ashwagandha live-style output when stress context stays primary and extract identity remains practical", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /stress|mood|resilience|calm/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /extract identity|comparison set|withania/i,
  );
});
...
test("scientific background accepts ginseng live-style output when species detail keeps energy framing specific", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /energy|fatigue|stimulant-style/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /species|extract|ginseng alone|comparison/i,
  );
});
...
test("scientific background accepts green tea extract live-style output when catechin detail stays primary and weight claims stay bounded", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /catechin|EGCG|extract-detail/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /concentration detail|comparison set|green tea extract/i,
  );
});
...
test("dha fallback is clearly differentiated from epa framing", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /brain[- ]and[- ]eye|brain and eye context|brain and eye/i,
  );
  assert.match(
    result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /EPA and DHA should not be treated as interchangeable|same total omega-3/i,
  );
  assert.doesNotMatch(
    result.scientificBackground.sections[0]?.summary ?? "",
    /triglyceride and lipid-marker research, which makes this the clearest evidence lane/i,
  );
});
...
test("epa fallback now uses lipid and triglyceride evidence grounding as the primary omega-3 lane", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /triglyceride|lipid endpoints|comparison lane/i,
  );
  assert.match(
    result.scientificBackground.sections[0]?.evidenceRead ?? "",
    /triglyceride|lipid|broader cardiovascular packaging/i,
  );
  assert.match(
    result.scientificBackground.sections[0]?.shopperMeaning ?? "",
...
test("vitamin c fallback keeps iron context as a specific lane instead of a generic claim", async () => {
  ...
  assert.match(
    ironSection?.summary ?? "",
    /iron co-administration|use-case-specific context|iron-absorption lane|context-specific comparison/i,
  );
  assert.match(
    ironSection?.shopperMeaning ?? "",
    /paired nutrient use|iron context|iron-absorption wording|narrower comparison lane|includes iron|use case/i,
  );
});
...
test("magnesium and vitamin d research mode get longer execution budgets than the generic research profile", () => {
  ...
  const magnesiumProfile =
    resolveScientificBackgroundExecutionProfile(magnesiumPlan);
  const vitaminDProfile =
    resolveScientificBackgroundExecutionProfile(vitaminDPlan);
  const vitaminCProfile =
    resolveScientificBackgroundExecutionProfile(vitaminCPlan);
  ...
  assert.ok(
    magnesiumProfile.backgroundRefreshTimeoutMs >
      vitaminDProfile.backgroundRefreshTimeoutMs,
  );
...
test("calcium, zinc, and iron research mode get dedicated execution budgets", () => {
  ...
  const calciumProfile =
    resolveScientificBackgroundExecutionProfile(calciumPlan);
  ...
  assert.ok(
    zincProfile.backgroundRefreshTimeoutMs >
      ironProfile.backgroundRefreshTimeoutMs,
  );
...
test("melatonin and b-vitamin research mode get longer execution budgets than the generic research profile", () => {
  ...
  const melatoninPlan = planScientificBackgroundSections({
    context,
    selectedIngredientName: "Melatonin",
  });
  ...
  const melatoninProfile =
    resolveScientificBackgroundExecutionProfile(melatoninPlan);
  ...
  assert.equal(vitaminCPlan.mode, "research_mode");
  assert.ok(
    melatoninProfile.backgroundRefreshTimeoutMs > melatoninProfile.timeoutMs,
  );
...
test("botanical families get longer execution budgets than the generic research profile", () => {
  ...
  const curcuminProfile =
    resolveScientificBackgroundExecutionProfile(curcuminPlan);
  const ashwagandhaProfile =
    resolveScientificBackgroundExecutionProfile(ashwagandhaPlan);
  const ginsengProfile =
    resolveScientificBackgroundExecutionProfile(ginsengPlan);
  const greenTeaProfile =
    resolveScientificBackgroundExecutionProfile(greenTeaPlan);
  const vitaminCProfile =
    resolveScientificBackgroundExecutionProfile(vitaminCPlan);
...
test("planner assigns dedicated section packs to magnesium, vitamin D, calcium, iron, melatonin, b-vitamin, and expanded ingredient families", () => {
  ...
  assert.deepEqual(
    planScientificBackgroundSections({
      context: turmericContext,
      selectedIngredientName: "Turmeric Extract (Curcuma longa)",
    }).sections.map((section) => section.heading),
    [
  ...
  assert.deepEqual(
    planScientificBackgroundSections({
      context: coq10Context,
      selectedIngredientName: "Coenzyme Q10 (Ubiquinol)",
    }).sections.map((section) => section.heading),
    [
  ...
  assert.deepEqual(
    planScientificBackgroundSections({
      context: berberineContext,
      selectedIngredientName: "Berberine HCl",
    }).sections.map((section) => section.heading),
    [
  ...
  assert.deepEqual(
    planScientificBackgroundSections({
      context: nacContext,
      selectedIngredientName: "N-Acetyl-Cysteine",
    }).sections.map((section) => section.heading),
    [
  ...
  assert.deepEqual(
    planScientificBackgroundSections({
      context: collagenContext,
      selectedIngredientName: "Marine Collagen Peptides",
    }).sections.map((section) => section.heading),
    [
  ...
  assert.deepEqual(
    planScientificBackgroundSections({
      context: electrolyteContext,
      selectedIngredientName: "Electrolyte Mineral Stack",
    }).sections.map((section) => section.heading),
    [
      "Hydration context",
...
test("new family fallbacks stay specific and do not collapse back to generic prose", async () => {
  ...
  const magnesiumResult = await compileScientificBackgroundAsync(
    magnesiumContext,
    "Magnesium (as Magnesium Glycinate)",
  );
  const vitaminDResult = await compileScientificBackgroundAsync(
    vitaminDContext,
    "Vitamin D3 (as Cholecalciferol)",
  );
  const vitaminCResult = await compileScientificBackgroundAsync(
    vitaminCContext,
    "Vitamin C (as Ascorbic Acid)",
  );
  const calciumResult = await compileScientificBackgroundAsync(
    calciumContext,
    "Calcium (as Calcium Citrate)",
  );
  const zincResult = await compileScientificBackgroundAsync(
    zincContext,
    "Zinc (as Zinc Picolinate)",
  );
  const ironResult = await compileScientificBackgroundAsync(
    ironContext,
    "Iron (as Ferrous Bisglycinate Chelate)",
  );
  const melatoninResult = await compileScientificBackgroundAsync(
    melatoninContext,
    "Melatonin",
  );
  const b12Result = await compileScientificBackgroundAsync(
    b12Context,
    "Vitamin B12 (as Methylcobalamin)",
  );
  const folateResult = await compileScientificBackgroundAsync(
    folateContext,
    "Folate (as 5-MTHF)",
  );
  const b6Result = await compileScientificBackgroundAsync(
    b6Context,
    "Vitamin B6 (as Pyridoxal-5-Phosphate)",
  );
  const curcuminResult = await compileScientificBackgroundAsync(
    curcuminContext,
    "Curcumin C3 Complex",
  );
  const turmericResult = await compileScientificBackgroundAsync(
    turmericContext,
    "Turmeric Extract (Curcuma longa)",
  );
  const coq10Result = await compileScientificBackgroundAsync(
    coq10Context,
    "Coenzyme Q10 (Ubiquinol)",
  );
  const berberineResult = await compileScientificBackgroundAsync(
    berberineContext,
    "Berberine HCl",
  );
  const nacResult = await compileScientificBackgroundAsync(
    nacContext,
    "N-Acetyl-Cysteine",
  );
  const collagenResult = await compileScientificBackgroundAsync(
    collagenContext,
    "Marine Collagen Peptides",
  );
  const proteinResult = await compileScientificBackgroundAsync(
    proteinContext,
    "Whey Protein Isolate",
  );
  const fiberResult = await compileScientificBackgroundAsync(
    fiberContext,
    "Apple Fiber",
  );
  const electrolyteResult = await compileScientificBackgroundAsync(
    electrolyteContext,
    "Electrolyte Mineral Stack",
  );
  const ashwagandhaResult = await compileScientificBackgroundAsync(
    ashwagandhaContext,
    "Ashwagandha (KSM-66)",
  );
  const ginsengResult = await compileScientificBackgroundAsync(
    ginsengContext,
    "Panax Ginseng Extract",
  );
  const greenTeaResult = await compileScientificBackgroundAsync(
    greenTeaContext,
    "Green Tea Extract (EGCG)",
  );

  assert.match(
    magnesiumResult.scientificBackground.sections[1]?.summary ?? "",
    /citrate|oxide|form-level comparison/i,
  );
  assert.match(
    magnesiumResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /form|magnesium amount|comparison set/i,
  );
  assert.match(
    vitaminDResult.scientificBackground.sections[0]?.summary ?? "",
    /bone and calcium-regulation|vitamin d positioning/i,
  );
  assert.match(
    vitaminDResult.scientificBackground.sections[1]?.summary ?? "",
    /immune|broader health|bone-and-calcium lane/i,
  );
  assert.match(
    vitaminDResult.scientificBackground.sections[2]?.summary ?? "",
    /dose|baseline status|label setting|similar-looking products/i,
  );
  assert.match(
    vitaminCResult.scientificBackground.sections[0]?.summary ?? "",
    /antioxidant-and-immune|immune-everything|vitamin C/i,
  );
  assert.match(
    vitaminCResult.scientificBackground.sections[1]?.summary ?? "",
    /collagen|tissue-support|healing context/i,
  );
  assert.match(
    vitaminCResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /formula actually includes iron|co-administration|narrower comparison lane/i,
  );
  assert.match(
    calciumResult.scientificBackground.sections[1]?.summary ?? "",
    /citrate-versus-carbonate|bioavailability literature|calcium form comparison/i,
  );
  assert.match(
    calciumResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /vitamin D|broader formula|direct substitutes|lead story/i,
  );
  assert.match(
    zincResult.scientificBackground.sections[0]?.summary ?? "",
    /immune-function context|zinc lane|immune-everything/i,
  );
  assert.match(
    zincResult.scientificBackground.sections[1]?.summary ?? "",
    /skin and barrier|second lane|dermatology|beauty/i,
  );
  assert.match(
    ironResult.scientificBackground.sections[0]?.summary ?? "",
    /supplementation and status-related lens|iron products/i,
  );
  assert.match(
    ironResult.scientificBackground.sections[1]?.summary ?? "",
    /bisglycinate|sulfate|form-aware/i,
  );
  assert.match(
    ironResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /elemental iron amount|single-form iron|broader blend/i,
  );
  assert.equal(melatoninResult.scientificBackground.sections.length, 2);
  assert.match(
    melatoninResult.scientificBackground.sections[0]?.summary ?? "",
    /sleep timing and onset|circadian timing/i,
  );
  assert.match(
    melatoninResult.scientificBackground.sections[1]?.summary ?? "",
    /dose, timing, and use context|shopper needs|similar-looking products/i,
  );
  assert.match(
    b12Result.scientificBackground.sections[0]?.summary ?? "",
    /supplementation and status-related|b12 products|multi-B comparison lane|standalone B12 product/i,
  );
  assert.match(
    b12Result.scientificBackground.sections[1]?.summary ?? "",
    /nerve|red-blood-cell|broader energy language/i,
  );
  assert.match(
    b12Result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /form|comparison bucket|stated amount/i,
  );
  assert.match(
  ...
  assert.match(
    folateResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /folic acid|5-MTHF|folate disclosure|comparison|exact folate line|multi-B products|same bucket|broader B-complex/i,
  );
  assert.match(
  ...
  assert.match(
    ...
    /dose|formula role|comparison bucket|larger b-vitamin stack|comparison set|whole B-complex profile|isolating the B6 row/i,
  );
  assert.match(
    curcuminResult.scientificBackground.sections[1]?.summary ?? "",
    /extract|curcuminoid|standardized/i,
  );
  assert.match(
    curcuminResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /extract detail|broadest promise|package/i,
  );
  assert.match(
    turmericResult.scientificBackground.sections[0]?.summary ?? "",
    /turmeric|whole-root|broader/i,
  );
  assert.match(
    turmericResult.scientificBackground.sections[1]?.summary ?? "",
    /extract|curcuminoid|standardization/i,
  );
  assert.match(
    turmericResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /formula map|lead active|companion|curcumin|interchangeable|extracts/i,
  );
  assert.match(
    coq10Result.scientificBackground.sections[0]?.summary ?? "",
    /energy|coq10|coenzyme q10|mitochond/i,
  );
  assert.match(
    coq10Result.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /ubiquinol|ubiquinone|form|comparison/i,
  );
  assert.match(
    berberineResult.scientificBackground.sections[0]?.summary ?? "",
    /glucose|metabolic|berberine/i,
  );
  assert.match(
    berberineResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /dose|berberine|combo|label/i,
  );
  assert.match(
    nacResult.scientificBackground.sections[0]?.summary ?? "",
    /glutathione|precursor|nac/i,
  );
  assert.match(
    nacResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /dose|use context|comparison|label/i,
  );
  assert.match(
    collagenResult.scientificBackground.sections[0]?.summary ?? "",
    /collagen|connective|skin/i,
  );
  assert.match(
    collagenResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /source|type|collagen|comparison/i,
  );
  assert.match(
    proteinResult.scientificBackground.sections[0]?.summary ?? "",
    /protein|muscle|recovery/i,
  );
  assert.match(
    proteinResult.scientificBackground.sections[1]?.summary ?? "",
    /satiety|meal-support|broader/i,
  );
  assert.match(
    proteinResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /source|blend|grams|protein|comparison/i,
  );
  assert.match(
    fiberResult.scientificBackground.sections[0]?.summary ?? "",
    /fiber|digestive|regularity/i,
  );
  assert.match(
    fiberResult.scientificBackground.sections[1]?.summary ?? "",
    /satiety|gut-environment|fiber type/i,
  );
  assert.match(
    fiberResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /fiber|source|solubility|comparison/i,
  );
  assert.match(
    electrolyteResult.scientificBackground.sections[0]?.summary ?? "",
    /hydration|electrolyte/i,
  );
  assert.match(
    electrolyteResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /electrolyte|balance|carbohydrate|comparison/i,
  );
  assert.match(
    ashwagandhaResult.scientificBackground.sections[0]?.summary ?? "",
    /stress|mood|resilience/i,
  );
  assert.match(
    ashwagandhaResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /extract identity|comparison set|ashwagandha/i,
  );
  assert.match(
    ginsengResult.scientificBackground.sections[0]?.summary ?? "",
    /energy|fatigue|ginseng/i,
  );
  assert.match(
    ginsengResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /species|extract|ginseng alone|head to head/i,
  );
  assert.match(
    greenTeaResult.scientificBackground.sections[0]?.summary ?? "",
    /catechin|EGCG|green tea extract/i,
  );
  assert.match(
    greenTeaResult.scientificBackground.sections[2]?.shopperMeaning ?? "",
    /concentration detail|comparison set|green tea extract/i,
  );
});
...
test("reviewed evidence variants sharpen vitamin C, B6, zinc, iron, and melatonin while zinc lozenges stay label-context", async () => {
  ...
  assert.match(
    vitaminCAltDeliveryResult.scientificBackground.sections[0]
      ?.shopperMeaning ?? "",
...
test("electrolyte drink mixes use family-specific label-context sections instead of the generic label-context fallback", async () => {
  ...
  assert.match(
    result.scientificBackground.sections[0]?.summary ?? "",
    /hydration|formula identity|electrolyte/i,
  );
  assert.match(
    result.scientificBackground.sections[1]?.shopperMeaning ?? "",
    /hydration line|balance|electrolyte|compare/i,
  );
});
...
test("7-keto, cla, and carnitine get family-specific research plans and longer budgets than generic research mode", () => {
  ...
  const sevenKetoProfile =
    resolveScientificBackgroundExecutionProfile(sevenKetoPlan);
  const claProfile = resolveScientificBackgroundExecutionProfile(claPlan);
  const carnitineProfile =
    resolveScientificBackgroundExecutionProfile(carnitinePlan);
  const vitaminCProfile =
    resolveScientificBackgroundExecutionProfile(vitaminCPlan);
  ...
  assert.ok(
    sevenKetoProfile.backgroundRefreshTimeoutMs >= sevenKetoProfile.timeoutMs,
  );
  assert.ok(claProfile.backgroundRefreshTimeoutMs >= claProfile.timeoutMs);
  assert.ok(
    carnitineProfile.backgroundRefreshTimeoutMs >= carnitineProfile.timeoutMs,
  );
});
...
test("5-htp, green tea extract, and omega-3 now use target-family live-writer profiles instead of the generic budget", () => {
  ...
  const htpPlan = planScientificBackgroundSections({
    context,
    selectedIngredientName: "5-HTP (5-hydroxytryptophan)",
  });
  const greenTeaPlan = planScientificBackgroundSections({
    context,
    selectedIngredientName: "Green Tea Extract (Camellia sinensis) (Leaf)",
  });
  const epaPlan = planScientificBackgroundSections({
    context,
    selectedIngredientName: "EPA (Eicosapentaenoic Acid)",
...
test("greens-style formulas avoid enzyme support lines as the default science ingredient", () => {
  const context = buildIngredientScienceContext({
    digest: buildDigest({
      ...
      actives: [
        {
          name: "Cytozymes Digestive Enzyme Assimilation",
          amount: 100,
          unit: "mg",
        },
...
test("title rescue rows recover higher-value science anchors for CLA, tea bags, probiotics, and mineral stacks", () => {
  ...
  assert.equal(
    probioticContext.anchorIngredient?.ingredientFamily,
    "probiotic_or_blend",
  );
  assert.match(
    brandBorneProbioticContext.ingredientRows[0]?.name ?? "",
    /\bprobiotic/i,
  );
  assert.equal(
    brandBorneProbioticContext.anchorIngredient?.ingredientFamily,
    "probiotic_or_blend",
  );
  assert.match(
    mineralStackContext.ingredientRows[0]?.name ?? "",
    /\bmagnesium\b/i,
  );
  assert.match(
    singleMineralContext.ingredientRows[0]?.name ?? "",
    /\bmagnesium\b/i,
  );
  assert.equal(
    singleMineralContext.anchorIngredient?.ingredientFamily,
    "magnesium",
  );
  assert.ok(
    mineralStackContext.ingredientRows.some((row) =>
      /\bcalcium\b/i.test(row.name),
    ),
  );
  assert.ok(
    mineralStackContext.ingredientRows.some((row) =>
      /\bzinc\b/i.test(row.name),
    ),
  );
});
...
test("greens and tea-bag products stay in label-context mode even when the rescued anchor looks supplement-like", () => {
  const greensContext = buildIngredientScienceContext({
    digest: buildDigest({
      ...
      dosageForm: "Powder",
      actives: [
        { name: "Magnesium (as Magnesium Citrate)", amount: 120, unit: "mg" },
      ],
    }),
    overlayClaims: null,
  });
  ...
  const greensPlan = planScientificBackgroundSections({
    ...
    selectedIngredientName:
      greensContext.anchorIngredient?.name ??
      "Magnesium (as Magnesium Citrate)",
  });
  const teaPlan = planScientificBackgroundSections({
    context: teaContext,
    selectedIngredientName:
      teaContext.anchorIngredient?.name ?? "Green Tea Extract",
  });
...
test("greens, tea bags, and juice powders are treated as label-context products from title alone", () => {
  const juicePowderContext = buildIngredientScienceContext({
    digest: buildDigest({
      ...
      dosageForm: "Capsule",
      actives: [
        { name: "Magnesium (as Magnesium Citrate)", amount: 50, unit: "mg" },
      ],
    }),
    overlayClaims: null,
  });
  ...
  const juicePowderPlan = planScientificBackgroundSections({
    ...
    selectedIngredientName:
      juicePowderContext.anchorIngredient?.name ?? "Magnesium",
  });
  const teaBagPlan = planScientificBackgroundSections({
    context: teaBagContext,
    selectedIngredientName:
      teaBagContext.anchorIngredient?.name ?? "Green Tea Extract",
  });
...
test("new metabolic families fall back with product-specific copy instead of generic research-direction prose", async () => {
  ...
  const sevenKetoResult = await compileScientificBackgroundAsync(
    context,
    "7-Keto (DHEA Acetate-7-one)",
  );
  const claResult = await compileScientificBackgroundAsync(
    context,
    "Conjugated Linoleic Acid (CLA) (from Safflower Oil)",
  );
  const carnitineResult = await compileScientificBackgroundAsync(
    context,
    "Acetyl-L-Carnitine HCl",
  );

  assert.match(
    sevenKetoResult.scientificBackground.sections[0]?.summary ?? "",
    /7-keto|metabolic-rate|body-composition/i,
  );
  assert.doesNotMatch(
    sevenKetoResult.scientificBackground.sections[0]?.summary ?? "",
  ...
  assert.match(
    claResult.scientificBackground.sections[0]?.summary ?? "",
    /body-composition|fatty-acid|cla/i,
  );
  assert.doesNotMatch(
    claResult.scientificBackground.sections[0]?.summary ?? "",
    /appears in several research directions/i,
  ...
  assert.match(
    carnitineResult.scientificBackground.sections[0]?.summary ?? "",
    /energy-transport|exercise-context|carnitine/i,
  );
  assert.doesNotMatch(
    carnitineResult.scientificBackground.sections[0]?.summary ?? "",
    /appears in several research directions/i,
