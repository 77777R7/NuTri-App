...
test('compileDecisionSupport never emits pending top-section facets when personalization context is missing', () => {
  const compiled = compileDecisionSupport({
    digest: buildOmegaDigest(),
    factsDigestHash: 'fixture-sr-omega-no-personalization',
    viewMode: 'details',
  });

  assert.equal(compiled.personalizedResultLane.goalFit.status, 'unavailable');
  assert.equal(compiled.personalizedResultLane.goalFit.analyzedGoalCount, 0);
  assert.deepEqual(compiled.personalizedResultLane.goalFit.goalCoverage, []);
  assert.equal(compiled.personalizedResultLane.personalInsight.status, 'unavailable');
  assert.equal(compiled.personalizedResultLane.allergyInsight.status, 'unavailable');
  assert.equal(compiled.personalizedResultLane.dosageContext.status, 'unavailable');
  assert.equal(compiled.personalizedResultLane.productStanding.status, 'unavailable');
  assert.doesNotMatch(compiled.personalizedResultLane.personalInsight.summary, /not attached yet/i);
  assert.doesNotMatch(compiled.personalizedResultLane.goalFit.summary, /not attached yet/i);
  assert.doesNotMatch(compiled.personalizedResultLane.allergyInsight.summary, /not attached yet/i);
  assert.doesNotMatch(compiled.personalizedResultLane.dosageContext.summary, /not attached yet/i);
  assert.doesNotMatch(compiled.personalizedResultLane.productStanding.summary, /not attached yet/i);
});

test('compileDecisionSupport keeps allergy unavailable without leaking pending while preserving analyzed goal coverage', () => {
  const compiled = compileDecisionSupport({
    digest: buildOmegaDigest(),
    factsDigestHash: 'fixture-sr-omega-missing-allergy-attach',
    viewMode: 'details',
    personalizationContext: buildPersonalizationContext({
      prioritizedGoals: ['energy', 'immunity', 'recovery'],
      selectedGoalKey: 'energy',
  ...
  assert.equal(compiled.personalizedResultLane.goalFit.status, 'ready');
  assert.equal(compiled.personalizedResultLane.goalFit.analyzedGoalCount, 3);
  assert.ok((compiled.personalizedResultLane.goalFit.goalCoverage ?? []).length > 0);
  assert.ok((compiled.personalizedResultLane.goalFit.goalCoverageSummary?.items ?? []).length > 0);
  assert.equal(compiled.personalizedResultLane.allergyInsight.status, 'unavailable');
  assert.equal(compiled.personalizedResultLane.dosageContext.status, 'ready');
  assert.equal(compiled.personalizedResultLane.productStanding.status, 'unavailable');
});
