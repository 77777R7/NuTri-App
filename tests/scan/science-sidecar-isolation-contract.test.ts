...
test('science UI uses new B/C sidecars and removes the legacy ingredient summary fetch', () => {
  ...
  assert.ok(dashboardSource.includes("const decisionDigestForScience = normalizeText(authoritativeDecisionTemplatePayload?.digest ?? '') || null;"));
  ...
  assert.ok(dashboardSource.includes("const decisionAuthorityScopeKey = decisionAuthorityState.status === 'ready'"));
  assert.ok(dashboardSource.includes("&& decisionSupportAuthorityGate === 'ready'"));
  ...
  assert.ok(dashboardSource.includes('if (scienceAuthorityWaiting) return null;'));
  assert.ok(dashboardSource.includes('if (!shouldLoadScienceSidecars) return;'));
  assert.ok(dashboardSource.includes('|| !decisionInputsHashForScience'));
  assert.ok(dashboardSource.includes('|| !personalizationScopeHashForScience'));
