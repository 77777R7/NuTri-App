...
const sharedWelcomeSource = readFileSync(
...
test('scan result emits result-ready and a single save-to-stack activation action', () => {
...
test('personalized-insights coach dismissal is tracked in the activation funnel', () => {
...
test('welcome and auth gate now frame NuTri as scan-first value', () => {
  assert.match(welcomeSource, /Scan a supplement and see fit and safety fast/);
  assert.match(welcomeSource, /Set up my first scan/);
  assert.match(sharedWelcomeSource, /Scan a supplement and see fit and safety fast/);
  assert.match(sharedWelcomeSource, /Set up my first scan/);
  assert.match(gateSource, /Scan a supplement/);
  assert.match(gateSource, /See fit and safety fast/);
  assert.match(gateSource, /Start Free Scan/);
  assert.match(gateSource, /trackOnboardingEvent\('first_scan_started'/);
});
