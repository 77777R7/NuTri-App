...
const OUT_DIR = getArg(
  "out-dir",
  path.join(ROOT, "output", "quality_marks", `igen_consumer_surface_fixture_bundle_${TODAY}`),
);
const OUT_JSON = getArg("out-json", path.join(OUT_DIR, "igen_consumer_surface_fixture_bundle.json"));
const OUT_MD = getArg("out-md", path.join(OUT_DIR, "igen_consumer_surface_fixture_bundle.md"));
...
const toFixtureRow = (row) => ({
  ...
  surfaceSignal: {
    type: "igen_official_signal",
    status: "ready",
    priority: toPriority(safeText(row?.deliveryLane)),
    deliveryLane: row?.deliveryLane ?? null,
    bundleLabel: row?.bundleLabel ?? null,
    ...
    signalState: row?.officialSignalState ?? "product_level_official_signal",
  },
...
const toMarkdown = (report) => {
  const lines = [];
  lines.push("# iGEN Consumer Surface Fixture Bundle");
  lines.push("");
...
const main = async () => {
  ...
  for (const row of fixtureRows) {
    ...
    increment(priorityCounts, safeText(row?.surfaceSignal?.priority) || "unknown");
  }

  const report = {
    schemaVersion: "igen_consumer_surface_fixture_bundle.v1",
    generatedAt: nowIso(),
    inputs: {
    ...
    integrationNote:
      "This fixture bundle is shaped for downstream consumer-surface integration. It preserves identity, display copy, evidence, and audit summary in one row-level payload.",
    rows: fixtureRows,
  };
