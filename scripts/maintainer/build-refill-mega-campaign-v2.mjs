...
const STAGING_PATH = getArg(
  "staging-json",
  path.join(ROOT, "output", "refill_mega_01", "run_r2", "current_staging_products.scrapling_merged.json"),
);
...
const OUT_DIR = getArg(
  "out-dir",
  path.join(ROOT, "output", "refill_mega_02", "miner_v2"),
);
...
const PROVEN_SUPPLEMENT_BRANDS = new Set(
  [
    "NOW Foods",
...
const TIER2_BRAND_DENYLIST = new Set(
  [
    "YumEarth",
    "Frontier Co-op",
    "Preggie",
...
const synthesizeQueueRow = (stagingRow, mergeRow) => {
  ...
  return {
    priorityLane: "P0_refill_miner_v2",
    recommendedAction: "official_fill_core_fields",
    rationale: "Queued partial row mined from full merge + staging corpus.",
...
const buildMarkdownSummary = (manifest) => {
  ...
  for (const [label, block] of [
    ...
    for (const [brandName, count] of Object.entries(block.summary.byBrand).slice(0, 15)) {
...
const main = async () => {
  ...
  for (const mergeRow of mergeReport?.rows ?? []) {
    ...
    const inTier1 = PROVEN_SUPPLEMENT_BRANDS.has(brandName);
    if (!inTier1 && TIER2_BRAND_DENYLIST.has(brandName)) continue;
