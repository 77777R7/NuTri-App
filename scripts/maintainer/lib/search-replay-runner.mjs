...
const buildSearchReplayRow = ({
  ...
  return {
    ...
    expectedIntent: scenario.expected?.search?.intent ?? null,
    expectedTier: scenario.expected?.search?.tier ?? null,
...
export const createSearchReplayReport = async ({
  ...
  for (const scenario of searchScenarios) {
    ...
    const url = buildSearchUrl({
      apiBaseUrl,
      query,
      category: scenario.input?.category ?? null,
      brand: scenario.input?.brand ?? null,
      page: scenario.input?.page ?? 1,
      limit: scenario.input?.limit ?? limit,
    });
    try {
...
export const renderSearchReplayMarkdown = (report) => {
  ...
  if (failures.length === 0) {
    ...
    for (const row of failures.slice(0, 12)) {
      lines.push(
        `- ${row.id}: ${row.reason}; query "${row.query}"; intent ${row.expectedIntent ?? "unknown"}; tier ${row.expectedTier ?? "unknown"}; expected ${row.expectedProductId}; rank ${row.rank ?? "missing"}; top ${row.topProductId ?? "none"}`,
      );
    }
  }
