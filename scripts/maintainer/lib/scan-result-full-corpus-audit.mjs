...
export const evaluateAiSummary = ({ type, product, payload, source, fallbackUsed, fallbackReason, backgroundRefreshPending, recommendedRetryAfterMs }) => {
  ...
  const blankFields = collectBlankFields(payload).filter(
    (field) =>
      !(
        type === "scientific_background" &&
        SCIENTIFIC_BACKGROUND_OPTIONAL_BLANK_FIELDS.has(field)
      ),
  );
  const unavailable = PLACEHOLDER_PATTERN.test(text) || !safeText(text);
  const genericCopyScore = scoreGenericText(text, product);
  const overclaimScore = scoreUnsafeOverclaimText(text);
...
const SCIENTIFIC_BACKGROUND_OPTIONAL_BLANK_FIELDS = new Set([
  "selectedDose",
  "closingNote",
]);

const isBoundedOverclaimContext = (text, index) => {
  const window = text
    .slice(Math.max(0, index - 90), Math.min(text.length, index + 90))
    .toLowerCase();
  const explicitNegation =
    /\b(?:do not|does not|should not|must not|cannot|not|never|avoid|without)\b[\s\S]{0,110}\b(?:cures?|treats?|treating|prevents?|prevention|guarantees?|detoxifies|replaces medication|clinically proven)\b/.test(
      window,
    );
  if (explicitNegation) return true;
  const comparisonFrame =
    /\b(?:treats?|treating)\b[\s\S]{0,90}\b(?:comparison|label|labels|formula|formulas|products?|lanes?|buckets?|substitutes?)\b/.test(
      window,
    ) ||
    /\b(?:comparison|label|labels|formula|formulas|products?|lanes?|buckets?|substitutes?)\b[\s\S]{0,90}\b(?:treats?|treating)\b/.test(
      window,
    );
  if (comparisonFrame) return true;
  const boundaryLanguage =
    /\b(?:avoid|avoids|avoiding|without|not|no|never|do not|does not|should not|must not|rather than|instead of|before)\b/.test(
      window,
    );
  const claimLanguage =
    /\b(?:claim|claims|wording|language|marketing|copy|promise|framing|slogan|hype|broad)\b/.test(
      window,
    );
  return boundaryLanguage && claimLanguage;
};

const scoreUnsafeOverclaimText = (text) => {
  const value = safeText(text);
  if (!value) return 0;
  const matches = [...value.matchAll(new RegExp(UNSAFE_OVERCLAIM_PATTERN, "gi"))];
  if (matches.length === 0) return 0;
  return matches.some((match) => !isBoundedOverclaimContext(value, match.index ?? 0))
    ? 3
    : 0;
};
...
const scoreFromText = (text, { product, needsBoundary = false, needsDose = false, needsCompare = false } = {}) => {
  ...
  if (scoreUnsafeOverclaimText(value) >= 3) return Math.min(score, 1);
  return score;
};
